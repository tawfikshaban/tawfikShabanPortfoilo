...
onMounted(() => {
  let tl = gsap.timeline();
  let midSplit = SplitText.create(".aboutPage > article .mid", {
    type: "words,lines",
    smartWrap: true,
    mask: "lines",
  });
  setTimeout(() => {
    ...
    tl.from(
      midSplit.lines,
      {
        ...
        mask: true,
        ease: "var(--secund-transition)",
        onComplete: () => midSplit.revert(),
      },
      0,
    );
    tl.from(
      ".aboutPage > article .bottom",
      {
        x: "-100vw",
        duration: 2,
        ease: "var(--secund-transition)",
  ...
  document.fonts.ready.then(() => {
    ...
    document
      .querySelector(".aboutPage >figure img:first-child")
      .addEventListener("mouseenter", (e) => {
        // e.target.style.clipPath = "inset(100% 0 100% 0)";
        let tl = gsap.timeline();
        tl.to(".aboutPage >figure img:nth-child(1)", {
          clipPath: "inset(0 0 100% 0)",
          duration: 0.5,
          ease: "var(--secund-transition)",
        });

        tl.to(".aboutPage >figure img", {
          clipPath: "inset(0 0 100% 0)",
          duration: 1,
          stagger: 0.4,
          ease: "var(--secund-transition)",
...
<template>
  <div class="aboutPage">
    ...
    <article>
      <h2 class="who top">who i am</h2>
      <h2 class="who mid">who i am</h2>
      <h2 class="who bottom">who i am</h2>

      <h2 class="details left">
        I’m a <span>Front-End Developer</span> focused on crafting fast, smooth,
        and visually engaging web experiences. I care about the small details
        that make a product feel intuitive, polished, and enjoyable to use.
      </h2>
      <h2 class="details right">
        <span>What I do</span>
        I build modern, interactive interfaces using tools and technologies that
        bring ideas to life Developing responsive and dynamic UIs Creating
        smooth animations and scroll-based experiences Optimizing performance
        and loading behavior Turning designs into clean, maintainable code
      </h2>
    </article>
    <figure>
      <NuxtImg src="/images/tawfik.jpg" alt="tawfik shaban" data-index="5" />
      <NuxtImg
        src="/images/first-image.jpg"
        alt="tawfik shaban"
        data-index="4"
...
.aboutPage {
  width: 100dvw;
  min-height: 100dvh;
  background-color: var(--aboutBlack-color);
  position: relative;
}
...
.who {
  ...
  justify-content: center;
  align-items: center;
  /* transition: all 0.8s var(--secund-transition); */
...
.aboutPage > article .details {
  position: absolute;
  width: 50vw;
  padding-top: 10px;
  /* transform: translateY(-50%); */
  font-family: "PowerGroteskTrial", monospace;
  font-weight: 300;
  font-size: 18px;
  text-transform: uppercase;
  line-height: 18px;
  ...
  /* letter-spacing: 1px; */
  display: none;
  transition: all 0.8s var(--main-transition);
  will-change: display, opacity;
}
.aboutPage > article .details span {
...
.aboutPage > article .details.left {
  left: 10px;
  top: 100px;
  clip-path: inset(0px 0px 0px 100%);
  transition: clip-path 0.8s var(--main-transition);
  will-change: clip-path;
}

.aboutPage > article .details.right {
  right: 10px;
  text-align: right;
...
.aboutPage > figure img {
  ...
  border-radius: 22px;
  clip-path: inset(100% 0 0 0);
  transition: clip-path 0.4s var(--secund-transition);
  will-change: clip-path;
}

.aboutPage > figure img:first-child {
  z-index: 5;
}
.aboutPage > figure img:nth-child(2) {
  z-index: 4;
}
.aboutPage > figure img:nth-child(3) {
  z-index: 3;
}
.aboutPage > figure img:nth-child(4) {
  z-index: 2;
}
.aboutPage > figure img:nth-child(5) {
  z-index: 1;
}
