...
const clients = ref([
  ...
  {
    name: "Lord Restaurant",
    projectName: "Lord Restaurant Food Menu",
    description:
  ...
  {
    name: "Opulent",
    projectName: "Opulent Perfumes Store",
    description:
...
onMounted(() => {
  ...
  mobileScreen.value = document.body.clientWidth >= 786 ? true : false;
  ...
  document.fonts.ready.then(() => {
    let midSplit = SplitText.create(
      ".clientsPage > article .clientsLoading:nth-child(2)",
      {
        type: "words,lines",
        smartWrap: true,
        mask: "lines",
    ...
    setTimeout(() => {
      ...
      tl.to(".clientsPage > article .clientsLoading", {
        opacity: 0,
        duration: 0.5,
        ease: "var(--secund-transition)",
      });
...
<template>
  <div class="clientsPage">
    ...
    <main>
      ...
      <div v-if="mobileScreen" class="project-thumbnail">
        <div class="thumbnail" v-for="(img, index) in clients" :key="index">
          <NuxtImg :src="img.img" alt="image" />
        </div>
      </div>
    </main>

    <article class="clientsLoadingContainer">
      <h2 class="clientsLoading top">my clients</h2>
      <h2 class="clientsLoading mid">my clients</h2>
      <h2 class="clientsLoading bottom">my clients</h2>
    </article>
...
.clientsPage main > ul li:hover {
  cursor: pointer;
}

.clientsPage main > ul li::before,
.clientsPage main > ul li::after {
  content: "";
  position: absolute;
  height: 0.5px;
  left: 0;
...
.clientsPage main > ul li > article {
  width: 100%;
  height: 100px;
  display: flex;
...
.clientsPage main > ul li > article > article {
  width: 30%;
  height: 100%;
  display: flex;
  justify-content: center;
...
.clientsPage main > ul li h2 {
  font-family: "owners", ui-monospace;
  text-transform: uppercase;
  line-height: 31px;
  letter-spacing: 1px;
  color: var(--secondary-color);
...
.clientsPage main .project-thumbnail {
  position: fixed;
  width: 400px;
  height: 250px;
  display: flex;
...
.clientsPage main .thumbnail {
  width: 100%;
  height: 100%;
}

.clientsPage main .thumbnail img {
  width: 100%;
  height: 100%;
  object-fit: fill;
  object-position: center;
...
@media screen and (max-width: 769px) {
  ...
  .clientsPage main > ul figure img {
    display: none;
  }
  .clientsLoading {
    font-size: 15vw;
